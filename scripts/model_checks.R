...
p_data <- ggplot() + 
  geom_line(data = dat_plot, aes(x = fYEAR, y = (COUNT/TOTAL), group = interaction(as.factor(TRANSECT_NO),SITE_NO, REEF_NAME)), 
            show.legend = FALSE, linewidth=.1, col="grey30") + 
  geom_ribbon(data = dat_pred, aes(x=fYEAR,ymin=.lower, ymax=.upper, group=1),alpha=.2, fill ="#0072B2FF") +
  geom_line(data = dat_pred, aes(x=fYEAR, y=pred, group=1),linewidth=.4) +
  facet_wrap(~tier, ncol=2) +
  ylab("Coral over") + xlab("Year")+  theme_pubr() +
...
p_nodata <-  ggplot() + 
   geom_line(data = dat_plot_no, aes(x = fYEAR, y = (TRUE_COUNT/TOTAL)*100, group = interaction(as.factor(TRANSECT_NO),SITE_NO,REEF_NAME)), 
            show.legend = FALSE, linewidth=.1, col="red", linetype = "dashed") +
  geom_ribbon(data = dat_pred_without, aes(x=fYEAR,ymin=.lower*100, ymax=.upper*100, group=1),
           alpha=.2, fill ="#D68A8A") +
...
p_traj <- p_data / p_nodata + plot_annotation(tag_levels = 'A')
p_traj 
