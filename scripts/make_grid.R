#' @title Create tiers and save the spatial layer
...
ggsave(filename = paste0(title_of_run,"/report/extra/grid_reef_loc_",grid_size*100,"K.png"),
       plot = p_loc,  width=12, height=8)

# Combined plot

p_combined <- ggplot() +
  geom_sf(data = reef_loc, fill = "#00FFC2", col = "#00FFC2") +
  geom_sf(data = pred_layer, fill = "lightgray", col = "lightgray", alpha = 0.1) +
  geom_sf(data = X_sf, color = "red", size = 0.5) +
  theme_bw() +
  xlab("longitude") + 
  ylab("latitude")

ggsave(filename = paste0(title_of_run,"/report/extra/synthos_reef_",grid_size*100,"K.png"),
       plot = p_combined,  width=6, height=5)
